@echo off
rem  MM16DRead v0.1 * Status reader program for MM16D device
...
echo.
%PPC% %FPFLAG1% %FPFLAG2% %FPFLAG3% %FPFLAG4% mm16dread.lpr
echo.
...
if not exist "source\lib\%ARCH%-%OS%\mm16dread.exe" ( echo Error: firstly run "build.bat" to compile source code! & goto end )
...
config\mkshortcut.vbs /target:"%INSTDIR%\%NAME%.exe" /shortcut:"%USERPROFILE%\Desktop\MM16DRead"
echo.
...
rd /s "%INSTDIR%"
del /q "%USERPROFILE%\Desktop\MM16DRead.lnk"
goto end
